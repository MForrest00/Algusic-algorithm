...
class AppliedOctavedScale:
    ...
    def flat_altered_named_scale(self):
        if not hasattr(self, "_flat_altered_named_scale"):
            self._flat_altered_named_scale = [
                note for single_octave_scale in self.altered_named_scale for note in single_octave_scale
            ]
    ...
    def note_names(self):
        if not hasattr(self, "_note_names"):
            self._note_names = [
                [name for name, note in single_octave_scale] for single_octave_scale in self.named_scale
            ]
    ...
    def altered_scale(self):
        if not hasattr(self, "_altered_scale"):
            self._altered_scale = [
                [note for name, note in single_octave_scale] for single_octave_scale in self.altered_named_scale
            ]
    ...
    def flat_altered_scale(self):
        if not hasattr(self, "_flat_altered_scale"):
            self._flat_altered_scale = [
                note for single_octave_scale in self.altered_scale for note in single_octave_scale
            ]
    ...
    def altered_note_names(self):
        if not hasattr(self, "_altered_note_names"):
            self._altered_note_names = [
                [name for name, note in single_octave_scale] for single_octave_scale in self.altered_named_scale
            ]
    ...
    def flat_altered_note_names(self):
        if not hasattr(self, "_flat_altered_note_names"):
            self._flat_altered_note_names = [
                note for single_octave_scale in self.altered_note_names for note in single_octave_scale
            ]
    ...
    def generate_flat_tonic_indexes(self):
        tonic_indexes = [
            i
            for i, note_name in enumerate(self.chromatic_context.flat_note_names)
