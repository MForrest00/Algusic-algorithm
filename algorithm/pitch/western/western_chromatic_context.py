...
class WesternChromaticContext(TrueOctavedChromaticContext):
    ...
    def generate_note_name_components(self):
        ...
        ordered_note_names = (
            WesternChromaticContext.NOTE_NAMES[note_index:] + WesternChromaticContext.NOTE_NAMES[:note_index]
        )
    ...
    def generate_note_names(self):
        ...
        for i, single_octave_chromatic_scale in enumerate(self.chromatic_scale):
            scale = list()
            for note, note_name_components in zip(single_octave_chromatic_scale, self.generate_note_name_components()):
                if note is None:
                    scale.append(None)
                    continue
...
class WesternJustTemperedChromaticContext(UnequalTemperedTrueOctavedChromaticContext, WesternChromaticContext):
    ...
    def __str__(self):
        return (
            f"Classic western just tempered chromatic context in {self.anchor_note_name}, with "
            + f"{self.anchor_note_name}4 on {self.anchor_hz} hz"
        )


WesternEqualTempered432ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.0542, maximum_hz=7758.4577, anchor_hz=432.0
)
WesternEqualTempered434ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.1286, maximum_hz=7794.3765, anchor_hz=434.0
)
WesternEqualTempered436ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.2029, maximum_hz=7830.2953, anchor_hz=436.0
)
WesternEqualTempered438ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.2772, maximum_hz=7866.2141, anchor_hz=438.0
)
WesternEqualTempered440ChromaticContext = WesternEqualTemperedChromaticContext()
WesternEqualTempered442ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.4259, maximum_hz=7938.0517, anchor_hz=442.0
)
WesternEqualTempered444ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.5002, maximum_hz=7973.9704, anchor_hz=444.0
)
WesternEqualTempered446ChromaticContext = WesternEqualTemperedChromaticContext(
    minimum_hz=16.5745, maximum_hz=8009.8892, anchor_hz=446.0
)
